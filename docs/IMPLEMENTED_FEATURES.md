...
- **`tests/test_cards.py`** covers the US-1.1 grid detector (`CardBox`, `CardDetector._grid_boxes`) and the US-1.2 OpenCV contour fallback (`CardDetector._contour_boxes`): CardBox center math, degenerate images (empty/solid-color/missing-file → `[]`), a synthetic 2×2 grid (4 sorted, area-non-overlapping cells + strip-regression guard), a real `page-04.png` grid smoke test (≥2 boxes, sorted, >40% page coverage), a synthetic 3-rectangle contour image (≥3 boxes, sorted, non-overlapping), and a real `page-04.png` contour test (≥2 boxes, ≥2 distinct x-columns, sorted, non-overlapping). 22 tests total. Runs offline via `.venv/bin/python -m unittest tests.test_cards`.
...
- **Screenshots span multiple products.** `ImageService.detect_product_blocks` at `services/images.py:10` slices the page into equal horizontal strips, so each screenshot can contain a whole row of products instead of just one product card. This is the bug that triggered the redesign. A grid detector (`CardDetector._grid_boxes` in `services/cards.py`, US-1.1 ✅ DONE) now exists, and the OpenCV contour fallback `CardDetector._contour_boxes` (US-1.2 ✅ DONE, PR #3) handles dense real pages — but neither is wired into the adapter yet (wiring is US-3.1), and the unified `CardDetector.detect()` orchestrator is US-1.3 (pending); per-card regression (US-5.2) tightens density on dense real pages.
...
- **`numpy` is installed transitively but not declared explicitly.** `opencv-python>=4.8` is now declared in `pyproject.toml` (US-1.2 ✅ DONE, PR #3); `numpy` is still only transitively present (used by `services/cards.py` grid + contour paths).

---

## 10. Project Layout and Documentation

- **Package metadata and declared dependencies** live in `pyproject.toml` (declared deps: `httpx`, `Pillow`, `pydantic`, `pydantic-settings`, `rapidocr-onnxruntime`, `typer`, `opencv-python>=4.8` (US-1.2)). The CLI script is `leaflets`, mapped to `leaflet_automation.cli.main:app`.
